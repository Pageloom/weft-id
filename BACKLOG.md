...
**User Story:**
As an admin
I want to randomize the mandala on the branding page by clicking a button until I find one I like
So that I can save my preferred mandala as the tenant's light mode SVG logo

**Context:**

Mandalas are currently generated deterministically from the tenant UUID. The generator in `app/utils/mandala.py` accepts any string seed and produces consistent SVGs via a Mulberry32 PRNG. Today there is no way to explore different mandalas or persist a favorite. The branding page already supports mandala vs custom logo mode and handles SVG uploads with safety validation.

The flow: admin clicks "Randomize" to cycle through random seeds, previews the result live, and clicks "Save" to persist the chosen mandala as the custom light mode SVG logo. Since the saved mandala is an SVG generated by our own code, it can bypass the external SVG safety validation (no user-supplied content). The dark mode variant and favicon should also be generated and saved from the same seed for consistency.

**Acceptance Criteria:**

- [ ] "Randomize" button on the branding settings page generates a new mandala preview on each click
- [ ] Preview displays the light mode mandala at a visible size (not just the 40px nav icon)
- [ ] Each click uses a new random seed (not sequential, not predictable)
- [ ] "Save as Logo" button persists the displayed mandala as the custom light mode SVG logo
- [ ] Dark mode variant generated from the same seed is saved as the custom dark mode logo
- [ ] Favicon variant generated from the same seed is available (favicon preference follows existing logic)
- [ ] After saving, logo mode automatically switches to "custom" with the mandala SVGs as the logos
- [ ] Admin can continue randomizing and saving again (replaces previous)
- [ ] Event log entry when a mandala is saved as logo (`branding_logo_uploaded` with metadata indicating mandala source)
- [ ] API support: `POST /api/v1/branding/mandala/randomize` returns preview SVGs for a random seed; `POST /api/v1/branding/mandala/save` persists the mandala for a given seed

**Effort:** S
**Value:** High (Top requested branding feature, low implementation cost since generator already exists)
