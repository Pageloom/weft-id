...
**Status**: Feature complete on the `oidc-upstream` branch (2026-09-06, unreleased; the version bump happens on main at release time). Iteration 9 finished locally: docs + glossary, changelog entry, review findings logged to `.claude/ISSUES.md`, backlog item split and archived. The upstream-OIDC E2E test was split to its own backlog item rather than shipped here.
...
index + privileged-domains + mkdocs nav updates; CHANGELOG entry under Unreleased;
review pass (`make quality-all` green including 62 E2E; 4 findings logged to
