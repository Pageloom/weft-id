...
**User Story:**
As a super admin
I want Single Logout to work reliably for both SP-initiated and IdP-initiated flows
So that users are properly logged out across all federated applications

**Context:**

SLO was implemented as part of the SAML IdP feature set, but it has not been validated end-to-end. The implementation spans multiple layers (routers, services, utilities, XML signing) but there is zero E2E test coverage and it is unclear whether the admin UI and API provide complete support for configuring SLO on service providers.

This item is an audit-first effort: verify the full SLO configuration and execution path works from both ends, fill any gaps in UI/API support, then prove it with E2E tests.

**Audit Scope:**

- SP registration: Does importing SP metadata correctly capture the SP's SLO endpoint URL? Can it be set/edited manually?
- IdP metadata: Does our IdP metadata advertise the SLO endpoint so SPs know where to send LogoutRequests?
- SP detail page: Can admins see and edit the SLO URL for a registered SP?
- API: Does the SP create/update API expose SLO URL configuration?
- SP-initiated SLO flow: SP sends LogoutRequest to IdP, IdP clears session, IdP returns signed LogoutResponse, IdP propagates logout to other SPs
- IdP-initiated SLO flow: User logs out at IdP, IdP sends LogoutRequests to all SPs with active sessions, sessions cleared
- Session index correlation: SessionIndex from the original SSO assertion is used in LogoutRequests

**Acceptance Criteria:**

Configuration audit:
- [ ] SP metadata import extracts SLO endpoint URL (verify or fix)
- [ ] IdP metadata includes SingleLogoutService endpoint (verify or fix)
- [ ] SP detail page shows SLO URL (add if missing)
- [ ] SP detail page allows editing SLO URL (add if missing)
- [ ] API `POST /api/v1/service-providers` accepts SLO URL (verify or fix)
- [ ] API `PUT /api/v1/service-providers/{sp_id}` accepts SLO URL (verify or fix)
- [ ] Manual SP registration supports SLO URL entry (verify or fix)

E2E tests:
- [ ] SP-initiated SLO: user SSOs to SP, SP sends LogoutRequest, IdP clears session, LogoutResponse returned, user is logged out at IdP
- [ ] IdP-initiated SLO: user SSOs to SP, user logs out at IdP, SP session is cleared
- [ ] Session index correlation: LogoutRequest references the correct SessionIndex from the original assertion

Unit tests (if gaps found):
- [ ] Service-layer SLO processing with various edge cases
- [ ] SLO URL persistence through SP create/update lifecycle

**Key files:**
- `app/routers/saml_idp/slo.py` (SLO endpoints)
- `app/routers/saml_idp/admin.py` (SP admin UI)
- `app/services/service_providers/slo.py` (SLO service logic)
- `app/services/service_providers/crud.py` (SP create/update)
- `app/utils/saml_slo.py` (SLO XML building)
- `app/utils/saml_idp.py` (metadata generation, SP metadata parsing)

**Effort:** M
**Value:** High (SLO is a core SAML capability with zero E2E validation)
