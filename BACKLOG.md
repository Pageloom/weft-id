...
So that I can develop API-first and then compose server-rendered pages using the same models and operations without duplication or HTTP overhead

**Background:**

Current state:
```
[HTML Routes] → [Database Layer]
[API Routes]  → [Database Layer]
```

Both route types call the database layer directly, each handling their own model construction, validation, authorization, and business logic.
...
- [ ] New `app/services/` directory with domain-organized modules
- [ ] Service functions return Pydantic models (the API schemas from `app/schemas/`)
- [ ] Service layer handles **authorization** (can this user do this action?)
- [ ] Service layer handles business logic (validation, side effects like emails)
- [ ] Service layer is HTTP-agnostic (no knowledge of requests/responses)
- [ ] Routes handle **authentication** only (who is this user?) and inject requesting_user

**Exception Handling:**
- [ ] Custom exception hierarchy in `app/services/exceptions.py`
- [ ] Exceptions are HTTP-agnostic but translatable (include error code, message, optional details)
- [ ] API routes translate service exceptions to HTTPException
- [ ] HTML routes translate service exceptions appropriately (flash messages, error pages)

**API Routes Refactored:**
- [ ] API routes become thin wrappers around service calls
- [ ] API routes handle: HTTP parsing, authentication deps, response formatting
- [ ] All business logic moved out of API routes into service layer

**HTML Routes Refactored:**
- [ ] HTML routes call service layer (same as API)
- [ ] HTML routes receive typed Pydantic models
- [ ] No direct database layer calls from HTML routes

**Service Modules (matching current domains):**
- [ ] `services/users.py` - User CRUD, profile, email management, MFA
- [ ] `services/settings.py` - Tenant settings, privileged domains
- [ ] `services/oauth2.py` - OAuth2 clients, tokens
- [ ] `services/auth.py` - Authentication flows (if applicable to service layer)

**Authorization Model:**

The split:
- **Authentication** (route layer): "Who is making this request?"
  - FastAPI dependencies (`require_user`, `require_admin_api`, etc.) identify the caller
  - Injects `requesting_user: dict` into the service call

- **Authorization** (service layer): "Can this user perform this action?"
  - Service checks role, ownership, tenant isolation
  - Raises `ForbiddenError` if not permitted

Why authorization in service layer:
1. Single source of truth - one check, enforced everywhere
2. Can't bypass - neither API nor HTML routes can skip auth
3. Testable - service tests verify auth without HTTP
4. DRY - no duplicated permission logic between API and HTML

**Migration Strategy:**
1. Create service layer infrastructure (exceptions, base patterns)
2. Migrate one domain completely (e.g., users) as pilot
3. Refactor API routes to use service layer
4. Refactor HTML routes to use service layer
5. Repeat for remaining domains
6. Remove any orphaned code in database layer that was only business logic

**Out of Scope:**
- Changing the database layer (it remains as data access)
- Changing the API contract (same endpoints, same schemas)
- Async/await refactoring (keep sync for now)

**Effort:** XL (significant refactoring across entire codebase)
