...
**Bearer token (SCIM)**
:   The credential WeftID presents to a downstream application to authenticate its SCIM push requests. Generated by WeftID, displayed in plaintext once at creation, and pasted into the downstream application's SCIM bearer-token field. Supports rotation with a 24-hour overlap window and instant revocation.
