...
def test_set_mfa_method(test_user):
    ...
    database.mfa.set_mfa_method(test_user["tenant_id"], test_user["id"], "email")
...
def test_enable_mfa(test_user):
    ...
    database.mfa.enable_mfa(test_user["tenant_id"], test_user["id"], "email")
...
def test_create_and_get_totp_secret(test_user):
    ...
    database.mfa.create_totp_secret(
        test_user["tenant_id"], test_user["id"], secret, test_user["tenant_id"]
    )

    # Retrieve TOTP secret
    totp_record = database.mfa.get_totp_secret(test_user["tenant_id"], test_user["id"], "totp")
...
def test_delete_totp_secrets(test_user):
    ...
    # Create TOTP secret
    database.mfa.create_totp_secret(
        test_user["tenant_id"], test_user["id"], "secret_to_delete", test_user["tenant_id"]
    )

    # Delete it
    database.mfa.delete_totp_secrets(test_user["tenant_id"], test_user["id"])
...
def test_create_totp_secret_replaces_existing(test_user):
    ...
    database.mfa.create_totp_secret(
        test_user["tenant_id"], test_user["id"], "first_secret", test_user["tenant_id"]
    )
    database.mfa.verify_totp_secret(test_user["tenant_id"], test_user["id"], "totp")

    # Create second secret (should replace and reset verification)
    database.mfa.create_totp_secret(
        test_user["tenant_id"], test_user["id"], "second_secret", test_user["tenant_id"]
    )
...
def test_create_and_list_backup_codes(test_user):
    ...
    database.mfa.create_backup_code(
        test_user["tenant_id"], test_user["id"], code_hash1, test_user["tenant_id"]
    )
    database.mfa.create_backup_code(
        test_user["tenant_id"], test_user["id"], code_hash2, test_user["tenant_id"]
    )
...
def test_verify_backup_code(test_user):
    ...
    # Verify it
    is_valid = database.mfa.verify_backup_code(test_user["tenant_id"], test_user["id"], code_hash)
...
def test_backup_code_cannot_be_reused(test_user):
    ...
    # First verification should succeed
    assert (
        database.mfa.verify_backup_code(test_user["tenant_id"], test_user["id"], code_hash) is True
    )

    # Second verification should fail (already used)
    assert (
        database.mfa.verify_backup_code(test_user["tenant_id"], test_user["id"], code_hash) is False
    )
...
def test_delete_backup_codes(test_user):
    ...
    # Delete all codes
    database.mfa.delete_backup_codes(test_user["tenant_id"], test_user["id"])
