...
The password strength policy checks passwords at set-time only. Four gaps remain:

1. **Same-password reuse.** A user forced to reset (by admin or breach detection) can re-enter
   the same password. Since we already have the current hash, the fix is simple: verify the new
   password doesn't match the current hash before accepting it. This is NOT an N-password history.
   NIST 800-63B discourages rotation-based history ("must differ from last N"). We only prevent
   reuse of the exact current password during a change.

2. **OAuth2 token survival.** When `password_reset_required` is set, sessions are invalidated
   but OAuth2 access/refresh tokens issued via the authorization_code flow survive. These should
   be revoked because they represent user-level grants that are no longer trustworthy.

3. **Continuous HIBP monitoring.** A password that was clean at set-time can appear in a breach
   later. To detect this without storing reversible password data, store two values at set-time:
   `hibp_prefix` (first 5 hex chars of SHA1, plaintext) and `hibp_check_hmac`
   (HMAC-SHA256 of the full SHA1, keyed with an HKDF-derived key with purpose "hibp").
   A background job periodically queries HIBP with the prefix, reconstructs candidate SHA1s
   from the returned suffixes, computes their HMACs, and compares. On match, sets
   `password_reset_required`. The prefix alone maps to ~500-800 hashes (no meaningful leakage).
   The HMAC cannot be reversed even with the key.

4. **Policy compliance enforcement.** When an admin tightens the password policy (raises minimum
   length or zxcvbn score), existing passwords that were set under a weaker policy should be
   flagged. Store the policy in effect at password-set time (`password_policy_length_at_set`,
   `password_policy_score_at_set`), not the actual password quality (which would leak information
   in a breach). When the policy changes, compare each user's stored policy values against the
   new policy. If theirs is weaker, set `password_reset_required`. This may produce false positives
   (a user with a 30-char password set under an 8-char minimum gets flagged when minimum rises to
   14), but that's an acceptable tradeoff.

**Acceptance Criteria:**

Same-password reuse prevention:
- [ ] Password change (self-service), forced reset, and HIBP-triggered reset all reject the
      new password if it matches the current password hash
- [ ] Clear, non-alarming error message: "New password must be different from your current password"

OAuth2 token revocation:
- [ ] When `password_reset_required` is set (by admin or HIBP job), all OAuth2 access and
      refresh tokens issued to the user via authorization_code flow are revoked
- [ ] Client credentials tokens are NOT affected (those belong to the client, not the user)
- [ ] Event logged: `oauth2_user_tokens_revoked` with reason (admin_forced, hibp_breach, policy_change)

Continuous HIBP monitoring:
- [ ] Migration adds `hibp_prefix` (char(5), nullable) and `hibp_check_hmac` (text, nullable)
      to the users table
- [ ] Both values are set at password-set time (onboarding, change, forced reset)
- [ ] New HKDF-derived key with purpose "hibp" in `app/utils/crypto.py`
- [ ] Background job queries HIBP API for each user's prefix, compares returned suffix HMACs
- [ ] On match: sets `password_reset_required`, clears `hibp_prefix` and `hibp_check_hmac`
- [ ] Job runs on a configurable schedule (default: weekly)
- [ ] Job respects HIBP rate limits (1 request per 1.5 seconds for free tier)
- [ ] Event logged: `password_breach_detected` with HIBP match count
- [ ] Admin notification (email to admins) when breaches are detected

Policy compliance enforcement:
- [ ] Migration adds `password_policy_length_at_set` (integer, nullable) and
      `password_policy_score_at_set` (integer, nullable) to the users table
- [ ] Both values are recorded at password-set time
- [ ] When admin updates password policy, compare all users' stored policy values against the
      new policy. If either dimension is weaker, set `password_reset_required`
- [ ] Event logged: `password_policy_compliance_enforced` with count of affected users
- [ ] Admin shown a confirmation before saving a stricter policy: "N users will be required to
...
- [ ] No same-password reuse check (user has forgotten their password, so re-entering it is not a concern)
