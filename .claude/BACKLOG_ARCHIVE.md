...
correlation via a per-connection `correlation_claim`), Google and Entra presets
(`hd` hosted-domain restriction; `oid` correlation for Entra), JIT provisioning,
claim-to-attribute mapping + mirroring with disconnect scrub, per-user unlink,
privileged-domain routing across both protocols (`determine_auth_route` moved to
protocol-neutral `services.auth_routing`), full admin UI + `/api/v1/oidc-upstream`
surface, opt-in email linking gated on `email_verified`, client secrets
Fernet-encrypted at rest, all outbound IdP HTTP through the SSRF guard, docs +
glossary.

**Split out at close (still in BACKLOG.md):** group claim handling (all vendors),
GitHub + Okta presets, and the upstream-OIDC browser E2E test. Four review
findings logged to ISSUES.md (host-run rate limiting inert, missing OIDC
rate-limit tests, manual endpoints API-only, `list_user_oidc_links` dead code).

**Original item (criteria for the deferred parts carried into the split items):**
...
**Status:** Complete (2026-07-06, branch `oidc-provider`, 5 iterations + final review)

**Summary:** WeftID became a spec-correct downstream OpenID Provider, additive on the existing OAuth2 provider. Ships per-tenant RSA signing keys (private key encrypted at rest, rotation with overlap) + `GET /.well-known/jwks.json`; RS256 ID tokens gated on an `oidc_enabled` client + the `openid` scope (plain OAuth2 clients unaffected); `GET /.well-known/openid-configuration` + `GET /userinfo` (tenant-host-scoped; userinfo confined to OIDC clients); a shared scope-gated claim assembler feeding both the ID token and userinfo (profile/email/groups); DAG-aware `groups` claims; authorize-time group access control (ungranted users denied before a code is issued, reusing the shared `sp_group_assignments` grant model via a third `oauth2_client_id` target); a full `/api/v1` + admin-UI management surface; docs; and comprehensive multi-tenant isolation tests. Final review fixed a refresh-token scope-drop, a bulk group-assign cross-tenant injection, gated userinfo on `oidc_enabled`, and bounded `scope`/`nonce`/admin-form inputs. Deferred to the OIDC Hardening item: signing-key rotation operator surface, plus logout/introspection/revocation/DCR/pairwise-sub/PAR (always out of scope here).
...
- [x] `GET /.well-known/openid-configuration` (tenant-scoped) returns a spec-compliant discovery document: `issuer`, `authorization_endpoint`, `token_endpoint`, `userinfo_endpoint`, `jwks_uri`, supported scopes, response types, grant types, subject types, `id_token_signing_alg_values_supported` (RS256), and claims supported
- [x] `GET /.well-known/jwks.json` (or discovery-advertised path) publishes the active public signing key(s) with stable `kid`; supports key rotation with overlap (old key served until tokens signed by it expire)
- [x] Token endpoint issues a signed `id_token` (RS256) when the `openid` scope is requested; omitted otherwise (backward compatible with existing opaque-token clients)
- [x] ID token contains: `iss` (tenant issuer), `sub` (stable WeftID user id), `aud` (client id), `exp`, `iat`, `auth_time`, `nonce` (echoed from the authorization request when supplied), and profile/email/group claims per requested scopes
- [x] `nonce` parameter accepted on the authorization request and bound into the ID token; replay of an authorization code is rejected (reuse existing single-use code semantics)
- [x] `GET /userinfo` (Bearer access token) returns claims consistent with the ID token and the granted scopes; rejects expired/revoked/invalid tokens
- [x] Scopes `openid profile email groups` supported and gate which claims are released; group claims sourced from the group system honoring effective (DAG) membership
- [x] OIDC login honors the existing app group-based access control (`sp_group_assignments` shared grant model); ungranted users are denied at authorize, not issued a token
- [x] Signing key provisioned with private key encrypted at rest; rotation procedure documented; `kid` stable per key
- [x] All discovery/JWKS/token/userinfo behavior is tenant-scoped under RLS; cross-tenant key/client/issuer leakage covered by tests
- [x] Per-client toggle or scope-gating so a relying party explicitly opts into OIDC (vs plain OAuth2); existing clients unaffected until they request `openid`
- [x] API surface under `/api/v1/` for managing OIDC-enabled clients, documenting all accepted fields (per API-first rule)
- [x] Audit events: `oidc_id_token_issued`, `oidc_userinfo_accessed`, `oidc_signing_key_rotated` (and reuse existing OAuth2 token/authorization events where applicable)
- [x] Documentation: `docs/admin-guide/identity-providers/oidc-provider-setup.md` (registering a downstream app, discovery URL, redirect URIs, scopes/claims, group claim behavior); glossary cross-links to the existing OAuth2/PKCE/OIDC entries
- [x] Test coverage: discovery document shape, JWKS publication + rotation, ID token signature/claims validation with a real RP verification path, nonce binding, userinfo, scope-gated claim release, access-control denial, multi-tenant isolation
...
- [x] `user_attributes` table with `user_id`, `tenant_id`, `attribute_key`, `value`, `source`, `source_idp_id`, `updated_at`
- [x] `tenant_attribute_config` table with `tenant_id`, `attribute_key`, `category`, `enabled`, `required`
- [x] Profile photo stored as `bytea` blob with unguessable public URL and cache headers
- [x] Manager stored as user reference (resolved) or raw text (unresolved)
- [x] Migration for all new tables

*Category opt-in UI:*
- [x] Settings page with category-level toggles (Contact, Professional, Location, Profile)
- [x] Per-attribute toggles within each category
- [x] Activating a category enables all its attributes
- [x] A category shows as "on" if any attribute within it is enabled
- [x] Super admins can mark individual attributes as required

*Editability and source tracking:*
- [x] Every attribute value tracks source (`idp`, `admin`, `self`)
- [x] IdP-sourced values are read-only for everyone with "Set by [IdP name]" indicator
- [x] Admin-sourced values editable by admins
- [x] Self-sourced values editable by the user (subject to `allow_users_edit_profile`)
- [x] IdP login overwrites with `source=idp` when assertion includes the attribute
- [x] Missing assertion attributes preserve existing values

*Profile photo:*
- [x] Users can upload their own photo
- [x] Admins can set photos on users
- [x] IdP-provided photo URLs fetched, resized, and stored
- [x] Served via unguessable public URL with cache headers
- [x] Falls back to mandala avatar when no photo is set

*Manager field:*
- [x] Resolved to user reference when possible, stored as raw text when not
- [x] Background job resolves unresolved managers periodically
- [x] Admins can manually set manager on any user

*Required field enforcement:*
- [x] Non-dismissable "profile incomplete" banner on dashboard linking to profile page
- [x] Admin view of users with incomplete required attributes
- [x] Bulk "force profile completion" action
- [x] Forced users can sign in but are redirected to profile and cannot navigate away until complete
- [x] User creation dialog surfaces enabled attributes with prepopulation suggestion

*SAML mapping:*
- [x] Per-IdP upstream attribute mapping configuration (IdP attribute name to WeftID standard key)
- [x] Standard attributes available in per-SP downstream assertion configuration
- [x] Standard SAML OIDs used by default, custom friendly names supported

*API:*
- [x] API endpoints for reading/writing user attributes
- [x] API endpoints for tenant attribute configuration
- [x] API endpoint for incomplete profiles list

*Audit:*
- [x] Attribute changes logged via `log_event()` with old/new values in metadata
- [x] Configuration changes (enable/disable/require) logged
- [x] `track_activity()` for read operations
