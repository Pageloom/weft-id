...
@router.get("/sp-logo/{sp_id}")
def serve_sp_logo(
    request: Request,
    sp_id: str,
    tenant_id: Annotated[str, Depends(get_tenant_id_from_request)],
):
    """Serve a service provider logo image. Unauthenticated, uses hostname-based tenant resolution.

    Supports ETag-based conditional requests and cache headers.
    """
    result = branding_service.get_sp_logo_for_serving(tenant_id, sp_id)

    if result is None:
        return Response(status_code=404)

    updated_str = str(result["updated_at"])
    etag_source = f"{sp_id}:{updated_str}"
    etag = hashlib.md5(etag_source.encode()).hexdigest()  # noqa: S324
    etag_header = f'"{etag}"'
