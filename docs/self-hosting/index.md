...
1. Edit `.env` and set `WEFT_VERSION` to the target version (e.g., `1.5.0`).
...
3. Edit `.env` and set `WEFT_VERSION` back to the previous version (e.g., `1.4.1`).
...
* `1.5.0` -- exact version (recommended for production)
* `1.5` -- latest patch for a minor version
...
|----------|-------------|
| `WEFT_VERSION` | Image tag to run (e.g., `1.5.0`). Pin to a specific version for stability. |
