...
def test_verify_email_public_success_new_user(test_tenant, mocker):
    ...
    assert "nonce=def456token" in location
    mock_verify.assert_called_once_with(test_tenant["id"], "email-123", "abc123token")
...
def test_verify_email_public_invalid_nonce(test_tenant, mocker):
    ...
    mock_get_email.return_value = {
        ...
        "email": "test@example.com",
        "verified_at": None,
        "verify_nonce": "correct_nonce",
        "set_password_nonce": "def456token",
    }

    client = TestClient(app)
    response = client.get("/verify-email/email-123/wrong_nonce", follow_redirects=False)
...
def test_set_password_page_renders(test_tenant, mocker):
    ...
    client = TestClient(app)
    response = client.get("/set-password?email_id=email-123&nonce=abc123token")
...
def test_set_password_page_invalid_nonce_redirects(test_tenant, mocker):
    ...
    mock_get_email.return_value = {
        ...
        "email": "test@example.com",
        "verified_at": datetime.now(UTC),
        "set_password_nonce": "current_nonce",  # DB has current, link has stale
    }
...
def test_set_password_success(test_tenant, mocker):
    ...
    mock_regenerate = mocker.patch(f"{SERVICES_EMAILS}.regenerate_set_password_nonce")
    ...
    mock_regenerate.assert_called_once_with(test_tenant["id"], "email-123")
...
def test_set_password_invalid_nonce_rejected(test_tenant, mocker):
    ...
    mock_get_email.return_value = {
        ...
        "email": "test@example.com",
        "verified_at": datetime.now(UTC),
        "set_password_nonce": "current_nonce",  # DB has current, form submits stale
    }
    mock_get_user.return_value = {"id": "user-123", "password_hash": None}
...
def test_set_password_passwords_dont_match(test_tenant, mocker):
    ...
    assert "nonce=abc123token" in location  # nonce must survive the error redirect
    mock_update.assert_not_called()
...
def test_set_password_logs_event(test_tenant, mocker):
    ...
    mocker.patch(f"{SERVICES_EMAILS}.regenerate_set_password_nonce")
...
def test_dashboard_redirects_when_force_profile_completion(test_user, mocker):
    """When the flag is set, /dashboard redirects to the profile page."""
    override_tenant(app, test_user["tenant_id"])

    mock_user = mocker.patch(f"{DEPS_AUTH}.get_current_user")
    flagged = dict(test_user)
    flagged["force_profile_completion"] = True
    mock_user.return_value = flagged
...
def test_dashboard_passes_missing_required_to_template(test_user, mocker):
    """Dashboard renders a non-empty banner when required keys are missing."""
    from fastapi.responses import HTMLResponse
    ...
    mock_compute = mocker.patch(
        ...
        return_value=[("job_title", False), ("department", True)],
    )
    ...
    response = client.get("/dashboard")
    assert response.status_code == 200
    # The locked-missing key (department) is filtered out; only job_title
    # belongs in the user-facing banner.
