...
I want to upload custom organization logos that replace the default mandala in the navigation
So that WeftId blends in with familiar organizational branding for end-users

**Context:**

WeftId currently shows an auto-generated mandala in the top navigation. Organizations want
their own logo displayed instead, so end-users see familiar branding. This feature adds a
branding settings page where admins can upload light-mode and dark-mode logo variants,
preview them, and optionally use them as favicons.

**Acceptance Criteria:**

**Logo Upload & Validation:**

- [ ] Upload page at Admin > Settings > Branding (admin+ access)
- [ ] Accept two logo slots: light mode and dark mode
- [ ] Accepted formats: PNG and SVG only
- [ ] PNG uploads must be square (width equals height), minimum 48x48px
- [ ] SVG uploads validated for square aspect ratio via viewBox
- [ ] Maximum file size: 256KB per file
- [ ] Clear error messages for validation failures (wrong format, not square, too large, too small)

**Preview & Display:**

- [ ] After upload, show side-by-side previews of how logos appear in light and dark mode
- [ ] If only a light-mode logo is uploaded, display it with a bright circle background in dark mode (matching the mandala treatment)
- [ ] If a dark-mode logo is uploaded, display it without the circle treatment
- [ ] When logo mode is "custom", the nav renders the logo via an `<img>` tag (enables browser caching)

**Logo Mode Toggle:**

- [ ] Radio control: "Mandala" (default) or "Custom logo"
- [ ] Switching to "Mandala" hides custom logos from the UI but preserves the uploaded files
- [ ] Switching back to "Custom logo" restores previously uploaded logos without re-upload

**Favicon:**

- [ ] Separate toggle: "Use logo as favicon" (default off)
- [ ] When enabled, serves the uploaded logo as the site favicon
- [ ] Recommend SVG format for favicon use (display hint on the page)
- [ ] Favicon toggle is independent of the nav logo toggle

**Serving & Caching:**

- [ ] Public endpoint `/branding/logo/{light|dark}` serves logo with correct `Content-Type`
- [ ] Endpoint is unauthenticated (required for favicon and `<img>` tag use)
- [ ] Response includes `Cache-Control` headers for browser caching
- [ ] Cache-busting mechanism when logos are updated (e.g., ETag or query param)

**Storage:**

- [ ] Logos stored in the database (not on disk) as binary data
- [ ] Single `tenant_branding` table, one row per tenant:
  - `tenant_id` (FK, unique), `logo_light` (bytea), `logo_light_mime` (varchar),
    `logo_dark` (bytea), `logo_dark_mime` (varchar), `logo_mode` (enum: mandala/custom),
    `use_logo_as_favicon` (boolean), `created_at`, `updated_at`

**API:**

- [ ] `POST /api/v1/branding/logo/{light|dark}` - upload a logo (multipart)
- [ ] `DELETE /api/v1/branding/logo/{light|dark}` - remove a logo
- [ ] `PUT /api/v1/branding/settings` - update logo mode and favicon toggle
- [ ] `GET /api/v1/branding` - retrieve current branding settings (without binary data)

**Event Logging:**

- [ ] Logo upload/delete logged as audit events
- [ ] Branding settings changes (mode toggle, favicon toggle) logged as audit events

**Effort:** M
**Value:** High (Direct end-user impact, table-stakes for enterprise identity platforms)
