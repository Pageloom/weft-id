...
- **Writes:** Templates (copy only), `docs/site/`, ISSUES.md, tech-writer log
...
The documentation lives at `docs/site/` and is organized hierarchically by audience and topic.
The site is static HTML/Markdown (tooling TBD on first run, could be MkDocs, Docusaurus, or
plain Markdown files that a static site generator consumes).
...
docs/site/
  ...
  api/
    index.md                        # API overview, authentication, link to /docs and /redoc
    # API reference is auto-generated by FastAPI at /docs (Swagger) and /redoc (ReDoc)
...
Screenshots are stored at `docs/site/assets/screenshots/` and named descriptively:
