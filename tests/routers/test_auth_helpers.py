...
def test_route_idp_oidc():
    """OIDC route type redirects to the OIDC login endpoint."""
    idp_id = str(uuid4())
    response = _call_route_after_email_verification(
...
def test_route_idp_oidc_jit():
    """OIDC JIT route type also redirects to the OIDC login endpoint."""
    idp_id = str(uuid4())
    response = _call_route_after_email_verification(
...
def test_route_idp_oidc_disabled():
    """OIDC disabled route type shows the idp_disabled error."""
    response = _call_route_after_email_verification(
        str(uuid4()), "user@example.com", "idp_oidc_disabled"
    )
    assert response.status_code == 303
...
# =============================================================================
# _route_without_verification Tests
# =============================================================================


def _call_route_without_verification(tenant_id, email, route_type, **kwargs):
    """Helper to invoke _route_without_verification with a mocked result."""
    from routers.auth._helpers import _route_without_verification
    ...
    with patch("routers.auth._helpers.saml_service.determine_auth_route", return_value=mock_result):
        return _route_without_verification(mock_request, tenant_id, email)


def test_route_without_verification_idp_oidc():
    """OIDC route redirects to OIDC login even without verification."""
    idp_id = str(uuid4())
...
def test_route_without_verification_idp_oidc_jit():
    """OIDC JIT route redirects to OIDC login even without verification."""
    idp_id = str(uuid4())
    response = _call_route_without_verification(
        str(uuid4()), "user@example.com", "idp_oidc_jit", idp_id=idp_id
    )
...
def test_route_without_verification_idp_oidc_disabled_no_disclosure():
    """OIDC disabled route falls back to password form (no disclosure)."""
    response = _call_route_without_verification(
        str(uuid4()), "user@example.com", "idp_oidc_disabled"
    )
    assert response.status_code == 303
    assert "show_password=true" in response.headers["location"]
    assert "idp_disabled" not in response.headers["location"]
