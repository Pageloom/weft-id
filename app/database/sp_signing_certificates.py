...
def get_certificates_needing_rotation_or_cleanup() -> list[dict]:
    ...
    Routes through the `list_sp_signing_certificates_for_rotation_unscoped()`
    SECURITY DEFINER function (migration 0055). The `sp_signing_certificates`
    ...
    return fetchall(
        UNSCOPED,
        """
        select id, sp_id, tenant_id, expires_at, rotation_grace_period_ends_at, action
        from list_sp_signing_certificates_for_rotation_unscoped()
        """,
    )
