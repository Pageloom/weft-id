...
migration runner
So that I can set up a fresh database quickly, apply incremental changes on demand, and
never worry about replaying 30+ sequential migration files
...
a lightweight Python migration runner (`migrate.py`), a `make migrate` target for on-demand
execution, and automatic migration on `docker-compose up` in dev environments only.

**Detailed technical plan:** [`.claude/references/migration-plan.md`](.claude/references/migration-plan.md)

**Key design decisions:**

1. **Dev-only auto-migration.** The `migrate` one-shot service only exists in a dev-specific
   compose override (`docker-compose.dev.yml`). It checks `IS_DEV=True` before running.
   Production deployments never auto-migrate. Migrations in prod are always explicit.

2. **On-demand utility.** `make migrate` runs the migration runner against a running database.
   This is the standard way to apply migrations in all environments.

3. **Migration log with success/failure tracking.** A `schema_migration_log` table records
   every migration attempt (not just successes). Each row captures: version, status
   (success/failed), timestamps, and full error details (Postgres error message + Python
   traceback) for failed runs. Successfully applied migrations cannot be rerun. Failed
   migrations can be retried after fixing the issue.
...
- [ ] Schema equivalence verified (`pg_dump` comparison between baseline-initialized DB and current)

**Migration runner (`db-init/migrate.py`):**
- [ ] Detects fresh vs existing database (checks for `schema_migration_log` table)
- [ ] Fresh DB: applies `schema.sql` baseline, records `baseline` as successful
- [ ] Existing DB: finds pending `.sql` files in `db-init/migrations/`, applies in lexicographic order
- [ ] Each migration runs in its own transaction. On success: records success in log. On failure: rolls back, records failure with full error details, exits non-zero
- [ ] Skips migrations that have a successful log entry (idempotent reruns)
- [ ] Allows rerun of migrations that only have failed log entries
- [ ] Connects as `postgres` superuser (same as current setup)
- [ ] Strips psql directives (`\` lines) so files work with both psycopg and `psql -f`

**Migration log table (`schema_migration_log`):**
- [ ] Columns: `id` (serial PK), `version` (text), `status` (text: 'success' or 'failed'), `started_at` (timestamptz), `completed_at` (timestamptz), `error_message` (text, nullable), `error_traceback` (text, nullable)
- [ ] System table: no RLS, no tenant_id, owned by postgres
- [ ] A migration is considered "applied" only if it has a row with `status = 'success'`
- [ ] Failed attempts are preserved for diagnostics (not deleted on retry)

**Dev auto-migration (`docker-compose.yml`):**
- [ ] `migrate` one-shot service added to `docker-compose.yml` (the dev compose file)
- [ ] Not added to `docker-compose.onprem.yml` (prod never auto-migrates)
- [ ] `app` and `worker` services depend on `migrate: service_completed_successfully`
- [ ] Auto-migration happens transparently on `make up` in dev

**On-demand utility:**
- [ ] `make migrate` runs the migration runner against a running database
- [ ] Works in any environment (dev, staging, prod) as long as the database is reachable
- [ ] `make db-init` wipes volume and reinitializes (baseline + migrations)

**Documentation and tests:**
- [ ] All existing tests pass
- [ ] CLAUDE.md updated with new migration workflow (creating migrations, applying, resetting)
