...
def get_idp_sp_certificates_needing_rotation_or_cleanup() -> list[dict]:
    ...
    Routes through the `list_idp_sp_certificates_for_rotation_unscoped()`
    SECURITY DEFINER function (migration 0055). The `saml_idp_sp_certificates`
    ...
    return fetchall(
        UNSCOPED,
        """
        select id, idp_id, tenant_id, expires_at, rotation_grace_period_ends_at, action
        from list_idp_sp_certificates_for_rotation_unscoped()
        """,
    )
