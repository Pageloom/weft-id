...
-- Standard user attribute expansion: data model + tenant attribute config.
--
-- This migration:
--   1. Creates user_attributes (EAV table for attribute values).
--   2. Creates tenant_attribute_config (per-tenant per-attribute toggles).
--   3. Adds users.force_profile_completion (default false) for iteration 7.
--   4. Seeds tenant_attribute_config for every existing tenant with the 14
--      standard attributes from the registry in app/constants/user_attributes.py.
--
...
CREATE TABLE user_attributes (
    ...
    CONSTRAINT user_attributes_idp_fkey
        FOREIGN KEY (source_idp_id)
    ...
    CONSTRAINT user_attributes_source_idp_consistency_check CHECK (
        (source = 'idp' AND source_idp_id IS NOT NULL)
        OR (source <> 'idp' AND source_idp_id IS NULL)
    ),
...
-- ============================================================================
-- 2. tenant_attribute_config (per-tenant per-attribute toggles)
-- ============================================================================
...
-- ============================================================================
-- 3. users.force_profile_completion
-- ============================================================================
ALTER TABLE users
    ADD COLUMN force_profile_completion BOOLEAN NOT NULL DEFAULT FALSE;

-- ============================================================================
-- 4. Seed tenant_attribute_config for every existing tenant
-- ----------------------------------------------------------------------------
...
-- send_to_sps_default=true. Surface stays invisible until tenants opt in.
-- ============================================================================
