...
**OIDC (OpenID Connect)**
:   An identity layer on top of [OAuth2](#oauth2). Where OAuth2 authorizes API access, OIDC adds a signed **ID token** and a standard set of identity claims, so an application can verify who the user is. WeftID acts as an OpenID Provider: an OIDC-enabled [App](admin-guide/integrations/apps.md) receives a signed ID token (via the [authorization code flow](#authorization-code-flow) with [PKCE](#pkce-proof-key-for-code-exchange)) plus scope-gated claims from the UserInfo endpoint. See [Sign in with WeftID (OIDC)](admin-guide/integrations/oidc-provider-setup.md).
...
:   An open standard for automating the exchange of user identity information between systems. WeftID is a SCIM 2.0 client: it pushes user and group changes to downstream applications so accounts are created, updated, and removed without manual intervention. See [Outbound SCIM Provisioning](admin-guide/service-providers/scim.md).
...
**Passkey**
:   A phishing-resistant credential that replaces both password and two-step verification in a single tap or biometric prompt. Backed by a device (laptop, phone, hardware key) or a synced credential provider (iCloud Keychain, 1Password, Google Password Manager). A user can register multiple passkeys and sign in with any of them. See [Passkeys](user-guide/passkeys.md).

**WebAuthn**
:   The W3C standard that underpins passkeys. Defines how browsers, credential providers, and relying parties (like WeftID) exchange public-key credentials. Passkeys are the user-facing name; WebAuthn is the protocol.

**Authentication strength policy**
:   A tenant setting controlling how strong a second factor WeftID requires. **Baseline** (default) accepts email one-time codes, TOTP, or passkeys. **Enhanced** rejects email codes: every user must have a TOTP authenticator or a passkey, and users without one are redirected to enroll on next sign-in. See [Authentication Policy](admin-guide/security/authentication-policy.md).

**Enhanced authentication enrollment**
:   The forced enrollment step a user goes through after an admin switches the tenant to the Enhanced strength policy. Users who previously relied on email one-time codes are sent to a setup page on next sign-in and cannot reach the dashboard until they register a TOTP authenticator or a passkey.
