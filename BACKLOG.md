...
## HMAC-Based XLSX Export Verification

**User Story:**
As an admin sharing an audit export with a compliance auditor,
I want to prove that the file was generated by WeftID and has not been tampered with,
So that the auditor can trust the export as authentic evidence.

**Context:**

XLSX has no native tamper-evident signature mechanism. The approach is a detached
HMAC: when WeftID generates an export, it computes HMAC-SHA256 of the encrypted
file bytes using a tenant-scoped key (derived via the existing HKDF infrastructure
in `app/utils/crypto.py`), stores the HMAC alongside the export record, and exposes
a verification endpoint. An admin uploads the file to verify authenticity.

**Acceptance Criteria:**

- [ ] On export generation, compute HMAC-SHA256 of the encrypted XLSX bytes using an HKDF-derived key scoped to the tenant
- [ ] Store the HMAC in the `export_files` record (new column)
- [ ] Add verification endpoint: `POST /api/v1/exports/{export_id}/verify` accepts file upload, recomputes HMAC, returns match/mismatch
- [ ] Admin UI: verification option on the background jobs page (upload file to check)
- [ ] Response clearly states "File is authentic, generated by WeftID on [date]" or "File does not match the original export"
